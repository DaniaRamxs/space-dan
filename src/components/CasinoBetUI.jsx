/**
 * CasinoBetUI.jsx
 * Componentes UI reutilizables para juegos de Casino:
 * - BettingScreen: pantalla inicial de apuesta
 * - ResultScreen: pantalla de resultado (ganó/perdió)
 * - CasinoHUD: HUD con balance y apuesta actual
 */
...
const green = '#00e676';
const red = '#ff1744';
const bg = 'rgba(10,8,28,0.95)';
const border = 'rgba(245,197,24,0.25)';

export function BettingScreen({ bet, setBet, balance, onPlay, isLoading, title, icon, description }) {
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        gap: 20, padding: '24px 20px', width: '100%', maxWidth: 420,
      }}
    >
      {/* Título */}
      ...
      {/* Balance */}
      <div style={{
        background: 'rgba(245,197,24,0.08)', border: `1px solid ${border}`,
      ...
      <div style={{ width: '100%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'center' }}>
          ...
          <input
            ...
            style={{
              ...
              border: `2px solid ${bet > balance ? red : border}`,
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'center', marginTop: 10, flexWrap: 'wrap' }}>
          {PRESETS.map(p => (
            <button
              key={p}
              onClick={() => setBet(Math.min(balance, p))}
              disabled={p > balance}
              style={{
                ...btnStyle(bet === p ? gold : 'rgba(255,255,255,0.1)'),
                color: bet === p ? '#000' : 'rgba(255,255,255,0.7)',
          ...
          <button
            ...
            style={{ ...btnStyle('rgba(255,100,0,0.3)'), fontSize: '0.8rem', padding: '6px 12px', borderRadius: 8, color: '#ff8c00' }}
          >MAX</button>
        </div>
      </div>
      ...
      <motion.button
        ...
        style={{
          ...
          background: canBet ? `linear-gradient(135deg, ${gold}, #e6a800)` : 'rgba(255,255,255,0.1)',
          color: canBet ? '#000' : 'rgba(255,255,255,0.3)',
          fontSize: '1.1rem', fontWeight: 900, letterSpacing: 1,
          cursor: canBet ? 'pointer' : 'not-allowed',
          boxShadow: canBet ? `0 0 30px rgba(245,197,24,0.35)` : 'none',
          transition: 'all 0.2s',
        }}
      >
        {isLoading ? '...' : `APOSTAR ◈ ${bet}`}
      </motion.button>

      {bet > balance && (
        <p style={{ color: red, fontSize: '0.8rem', margin: 0 }}>Balance insuficiente</p>
      )}
    </motion.div>
  );
}
...
export function ResultScreen({ result, bet, onPlayAgain, onClose }) {
  const { won, net, winAmount, message } = result;
  ...
  return (
    <motion.div
      ...
      style={{
        display: 'flex', flexDirection: 'column', alignItems: 'center',
        gap: 20, padding: '32px 24px', width: '100%', maxWidth: 380, textAlign: 'center',
      }}
    >
      <motion.div
        initial={{ scale: 0 }}
      ...
      <div style={{ display: 'flex', gap: 12, width: '100%' }}>
        ...
        <motion.button
          ...
          onClick={onPlayAgain}
          style={{
            ...btnStyle(`linear-gradient(135deg, ${gold}, #e6a800)`), flex: 2,
            padding: '12px', borderRadius: 12, color: '#000', fontSize: '1rem', fontWeight: 900,
          }}
...
export function CasinoHUD({ balance, bet, label = 'APUESTA' }) {
  return (
    <div style={{
...
function btnStyle(bg) {
  return {
    ...
    fontSize: '1rem', fontWeight: 700, transition: 'opacity 0.15s',
  };
}

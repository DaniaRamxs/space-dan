...
export default function ChessGame({ roomName, onClose, isTheater, onToggleTheater }) {
    ...
    useEffect(() => {
        ...
        const join = async () => {
            try {
                ...
                r.onLeave(() => console.log('[Chess] left room'));
            } catch (e) {
                if (!mounted) return;
    ...
    // Clear board selection whenever a move is confirmed by server
    useEffect(() => {
        setSelected(null);
        setLegalTargets([]);
    }, [tick]);
    ...
    }, [selected, legalTargets, isMyTurn, myColor, state?.gameState, room]);

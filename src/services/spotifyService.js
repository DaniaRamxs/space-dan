...
export const spotifyService = {
    ...
    // --- HELPERS ---
    _getRedirectUri() {
        const uri = process.env.NEXT_PUBLIC_SPOTIFY_REDIRECT_URI || `${window.location.origin}/spotify-callback`;
    ...
    async getAuthUrl() {
        ...
        const scope = 'user-read-currently-playing user-top-read user-read-private user-read-playback-state user-read-recently-played';
        const state = user.id;
    ...
    async syncCurrentSoundState() {
        try {
            ...
            if (!current || !current.item) {
                // Si teníamos una canción en curso, marcarla como pausada
                if (this._lastLoggedTrackId) {
                    await supabase.rpc('sync_user_sound_state', {
                        p_track_id: this._lastLoggedTrackId,
                        p_track_name: this._lastTrackName || '',
                        p_artist_id: this._lastArtistId || 'unknown',
                        p_artist_name: this._lastArtistName || 'Unknown Artist',
                        p_valence: null,
                        p_energy: null,
                        p_tempo: null,
                        p_emotional_label: null,
                        p_is_playing: false,
                        p_track_image_url: this._lastTrackImage || null
                    }).catch(() => {});
                }
                return null;
            }
            ...
            // Guardar info del track actual para poder sincronizar estado pausado
            this._lastTrackName = track.name;
            this._lastArtistId = track.artists[0]?.id || 'unknown';
            this._lastArtistName = track.artists[0]?.name || 'Unknown Artist';
            this._lastTrackImage = track.album?.images?.[0]?.url || null;

            // Registrar reproducción cuando cambia la canción
            if (isPlaying && track.id !== this._lastLoggedTrackId) {
                this._lastLoggedTrackId = track.id;
                supabase.from('track_play_logs').insert({
                    ...
                    if (logErr) console.warn('[Spotify] No se pudo registrar reproducción:', logErr.message);
                });
            }

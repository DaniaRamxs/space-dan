...
import { openUrl, isTauri, isCapacitor } from '../utils/openUrl';

// isNativeApp: Tauri o Capacitor (Android/iOS) — mismo flujo OAuth con polling
const isNativeApp = isTauri || isCapacitor;
...
export function useSpotify({ userId = null, isOwn = true } = {}) {
  ...
  const checkSpotifyConnection = async () => {
    try {
      ...
      const { data: connection, error: connErr } = await supabase
        ...
        .select('user_id, access_token, spotify_display_name, spotify_user_id')
        .eq('user_id', targetId)
        .maybeSingle();
      ...
      if (connection?.access_token) {
        setIsConnected(true);
        // Mostrar nombre de cuenta inmediatamente desde la BD (sin esperar API)
        if (connection.spotify_display_name || connection.spotify_user_id) {
          setSpotifyProfile({
  ...
  const connectSpotify = async () => {
    ...
    try {
      if (isNativeApp) {
        // En Tauri/Capacitor: usar la web de producción como callback.
        // No podemos usar localhost porque el navegador externo no llega al proceso nativo.
        // La web procesa el código OAuth y guarda los tokens en Supabase;
        // aquí detectamos la conexión vía polling.
        const { data: { user } } = await supabase.auth.getUser();
  ...
  const fetchSpotifyProfile = async () => {
    try {
      ...
      const data = await invokeSpotifyApi({ action: 'me', userId: targetId });
      return data || null;
    } catch (error) {
  ...
  const fetchSpotifyData = async () => {
    try {
      const [playing, artists, tracks, stats, profile] = await Promise.all([

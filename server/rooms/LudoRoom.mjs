...
export class LudoRoom extends GameRoom {
    ...
    initializeGame(options) {
        ...
        this.onMessage("join_game", (client, message) => {
            ...
            if (this.state.turnOrder.length >= 4) {
                this.startCountdown();
            }
        });
        ...
        this.onMessage("roll_dice", (client) => {
            ...
            if (this.state.waitingForMove || this.state.isRolling) return;
    ...
    executeMove(player, piece, roll) {
        ...
        if (player.pieces.every(p => p.status === "finished")) {
            ...
            if (this.state.winners.length >= this.state.turnOrder.length - 1) {
                this.endGame(this.state.winners[0] || player.userId);
                return;
            }
        }

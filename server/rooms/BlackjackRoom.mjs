...
export class BlackjackRoom extends GameRoom {
    static patchRateMs = 500; // turn-based — state changes only on actions
    maxPlayers = 24;
    ...
    initializeGame(options) {
        ...
        // Establecer primer turno al iniciar ronda
        this.onMessage("round_started", () => {
            const participating = Array.from(this.state.players.values()).filter(p => p.isParticipating);
            if (participating.length > 0) {
                this.state.currentTurn = participating[0].sessionId;
            }
        });
    }

...
export default class LudoScene extends Phaser.Scene {
    ...
    ensurePlayerPieces(player, sessionId) {
        ...
        if (!this.add) return; // Verificar que la escena esté inicializada
        ...
        player.pieces.forEach((piece, index) => {
            ...
            sprite.on('pointerdown', () => {
                if (this.room.sessionId === sessionId && this.state.currentTurn === sessionId && this.state.waitingForMove) {
                    const idx = sprite.getData('index');
                    this.room.send("move_piece", { index: idx });
                }
            });
    ...
    getGridPosition(color, position, pieceIndex) {
        ...
        const finalPosition = Math.min(position, path.length - 1);
        const finalCoords = path[finalPosition] || [7.5, 7.5];
    ...
    update(time, delta) {
        // Solo animar el efecto de pulso en las fichas que pueden moverse.
        // NO llamar updatePieces() aquí — eso crearía cientos de tweens por segundo.
        // updatePieces() se llama exclusivamente desde onStateChange.
        if (!this.state?.players) return;
        this.state.players.forEach((player, sessionId) => {
            if (!player.pieces) return;
            const isMyTurn = this.state.currentTurn === sessionId;
            player.pieces.forEach(piece => {
                ...
                const canMove = isMyTurn && this.state.waitingForMove && this.canMove(piece);
                if (canMove) {
                    // Pulso suave de escala en piezas movibles
                    sprite.setScale(1.15 + Math.sin(time / 200) * 0.1);
                }
            });
        });
    }

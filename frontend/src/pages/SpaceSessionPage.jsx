/**
 * SpaceSessionPage — Sala individual de un espacio
 *
 * Esta página:
 *   1. Se conecta al SpaceSessionRoom via useSpaceSession
 *   2. Si viene con ?activity=type:id&new=1, el host lanza la actividad automáticamente
 *   3. Renderiza ActivityRouter con la actividad activa
 *   4. Muestra VoiceModule como overlay opcional en la esquina
 *   5. En el lobby (sin actividad), muestra el ActivityLauncher para el host
 */

import { useEffect, useRef, useState } from 'react';
import { useParams, useSearchParams, useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { Crown, Users, ChevronLeft, Zap, Tv, BookOpen, Gamepad2, Music } from 'lucide-react';
...
// ─── Activity launcher (inline, solo host ve esto en el lobby) ────────────────
...
function ParticipantsBar({ participants, hostId }) {
  if (!participants.length) return null;
  return (
    <div className="flex items-center gap-2">
      {participants.slice(0, 6).map((p, i) => (
        <div key={p.userId || i} className="relative" title={p.username}>
          <img
...
export default function SpaceSessionPage() {
  ...
  const session = useSpaceSession(spaceId, {
    spaceName: spaceId,
    hostId: user?.id,
  });

  const {
    activity, voiceState, isHost, participants, status,
    launchActivity, stopActivity, toggleVoice, setMyVoiceStatus,
  } = session;

  const autoLaunchDone = useRef(false);

  // ── Auto-launch if ?activity=type:id&new=1 ────────────────────────────────
  useEffect(() => {
    if (autoLaunchDone.current) return;
    if (status !== 'connected') return;
    if (!isHost) return;

    const actParam = searchParams.get('activity'); // e.g. "anime:astro-party"
    ...
    toast.success(`${id} lanzado — ¡invita a tus amigos!`, { duration: 4000 });
  }, [status, isHost, searchParams, launchActivity]);

  // ── Host notification when someone joins ─────────────────────────────────
    ...
    <div className="relative min-h-full text-white">
      ...
      {!hasActivity && (
        <div className="sticky top-0 z-30 flex items-center justify-between border-b border-white/5 bg-[#030308]/80 px-4 py-3 backdrop-blur-xl sm:px-6">
          ...
          <ParticipantsBar participants={participants} hostId={hostParticipant?.userId} />
        </div>
      )}

      {/* ── Activity (full area when active) ────────────────────────────────── */}
              ...
              <>
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-sm text-white/40">
                    Esperando que el host elija una actividad...
                  </p>
                  {hostParticipant && (
      ...
      {/* ── VoiceModule — siempre en la esquina, nunca bloquea ──────────────── */}
      <div className="fixed bottom-20 right-4 z-[200] sm:bottom-6">
        <VoiceModule
          livekitRoom={voiceState.livekitRoom || spaceId}
          isEnabled={voiceState.active}
          isHost={isHost}
          onToggle={(active) => toggleVoice(active)}
          onVoiceStatus={(on) => setMyVoiceStatus(on)}
          profile={profile}
        />
      </div>
    </div>

...
function RadioPreview({ item }) {
    // Generate stable random heights once per mount using lazy state initialization
    const [stableHeights] = useState(() => [1, 2, 3, 4, 5].map(() => 30 + Math.random() * 70));
    return (
                ...
                <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 flex gap-0.5 h-3 items-end">
                    {[1, 2, 3, 4, 5].map((index, idx) => (
                        <div
                            ...
                            style={{ height: `${stableHeights[idx]}%`, animationDelay: `${index * 0.15}s` }}
                        />
                    ))}
                </div>
            </div>

...
-- 3. Desactivar RLS temporalmente para limpiar
...
DROP POLICY IF EXISTS "Roles visible to members" ON community_roles;
DROP POLICY IF EXISTS "Only owner can modify roles" ON community_roles;
...
DROP POLICY IF EXISTS "Channels visible to members" ON community_channels;
DROP POLICY IF EXISTS "Only owner can modify channels" ON community_channels;
...
DROP POLICY IF EXISTS "Member roles visible" ON community_member_roles;
DROP POLICY IF EXISTS "Only owner can remove member roles" ON community_member_roles;
DROP POLICY IF EXISTS "Only owner can modify member roles" ON community_member_roles;
...
-- 6. Crear políticas nuevas (sin IF NOT EXISTS)

-- community_roles
CREATE POLICY "roles_select" ON community_roles FOR SELECT 
    USING (EXISTS (SELECT 1 FROM community_members WHERE community_id = community_roles.community_id AND user_id = auth.uid()));

CREATE POLICY "roles_update" ON community_roles FOR UPDATE 
    USING (EXISTS (SELECT 1 FROM communities WHERE id = community_roles.community_id AND owner_id = auth.uid()));

CREATE POLICY "roles_delete" ON community_roles FOR DELETE 
    USING (EXISTS (SELECT 1 FROM communities WHERE id = community_roles.community_id AND owner_id = auth.uid()));

-- community_channels
CREATE POLICY "channels_select" ON community_channels FOR SELECT 
    USING (EXISTS (SELECT 1 FROM community_members WHERE community_id = community_channels.community_id AND user_id = auth.uid()));

CREATE POLICY "channels_update" ON community_channels FOR UPDATE 
    USING (EXISTS (SELECT 1 FROM communities WHERE id = community_channels.community_id AND owner_id = auth.uid()));

CREATE POLICY "channels_delete" ON community_channels FOR DELETE 
    USING (EXISTS (SELECT 1 FROM communities WHERE id = community_channels.community_id AND owner_id = auth.uid()));

-- community_member_roles
CREATE POLICY "member_roles_select" ON community_member_roles FOR SELECT 
    USING (EXISTS (SELECT 1 FROM community_members WHERE community_id = community_member_roles.community_id AND user_id = auth.uid()));

CREATE POLICY "member_roles_update" ON community_member_roles FOR UPDATE 
    USING (EXISTS (SELECT 1 FROM communities WHERE id = community_member_roles.community_id AND owner_id = auth.uid()));

CREATE POLICY "member_roles_delete" ON community_member_roles FOR DELETE 
    USING (EXISTS (SELECT 1 FROM communities WHERE id = community_member_roles.community_id AND owner_id = auth.uid()));
